...
def generate_presigned_url(
    bucket_name, blob_name, write_only=False, expiration=3600
) -> str:
    ...
    if write_only:
        perm = "put_object"
        return s3_client.generate_presigned_post(Bucket=bucket_name, Key=blob_name)

    response = s3_client.generate_presigned_url(
        perm, Params={"Bucket": bucket_name, "Key": blob_name}, ExpiresIn=expiration
    )
...
if __name__ == "__main__":
    ...
    try:
        presigned_url = generate_presigned_url(bucket_name, "secret_shares", "/", True)
        print(presigned_url)
    except Exception as e:
        print(e.format_exc())
        exit(1)
